...
import { Lightbulb, PenTool, Rocket, ArrowRight } from 'lucide-react';
  ...
  const steps = [
    {
      ...
      icon: <Lightbulb className="w-24 h-24 text-gray-100 absolute bottom-4 right-4 -z-10" />,
      rotation: "-rotate-2"
    },
    {
      number: "02",
      ...
      icon: <PenTool className="w-24 h-24 text-gray-100 absolute bottom-4 right-4 -z-10" />,
      rotation: "rotate-0"
    },
    {
      number: "03",
      ...
      icon: <Rocket className="w-24 h-24 text-gray-100 absolute bottom-4 right-4 -z-10" />,
      rotation: "rotate-2"
    }
  ];
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row items-stretch justify-center gap-6 relative">
          {steps.map((step, index) => (
            ...
            >
              <span className="text-lg font-semibold text-gray-400 block mb-4">
                {step.number}
              </span>
          ...
          <div className="hidden lg:block absolute top-0 left-1/3 -translate-x-1/2 -translate-y-8">
             <svg width="60" height="40" viewBox="0 0 60 40" fill="none" className="text-gray-300"><path d="M5 35C20 5 40 5 55 5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeDasharray="4 4"/></svg>
          </div>
        </div>
      </div>
    </section>
