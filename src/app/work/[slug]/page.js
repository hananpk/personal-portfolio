    ...
    <main className="bg-black min-h-screen text-white pb-24">
      ...
      <div className="max-w-6xl mx-auto px-8 mt-20 grid grid-cols-1 md:grid-cols-3 gap-16">
        <div className="md:col-span-2 space-y-24">
          {/* 1. Overview */}
          <section>
            <h2 className="text-blue-500 font-bold uppercase tracking-widest text-xs mb-4">
              Overview
            </h2>
            <p
              className="text-md md:text-lg text-zinc-300 leading-relaxed font-light"
          ...
          {project.features && (
            <section>
              <h2 className="text-blue-500 font-bold uppercase tracking-widest text-xs mb-8">
                Key Features
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {project.features.map((feature, index) => (
                  <div
                    key={index}
                    className="flex items-start gap-4 p-6 rounded-2xl bg-zinc-900/30 border border-white/10 hover:border-blue-500/30 transition-colors"
                  >
                ...
                {project.process.map((item, index) => (
                  <div key={index} className="flex gap-6 group">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-sm md:text-md text-zinc-400 leading-relaxed">
                        {item.description}
                      </p>
                    </div>
                  </div>
        ...
        <div className="space-y-12">
          <div className="sticky top-24">
            <h2 className="text-zinc-500 font-bold uppercase tracking-widest text-xs mb-6">
              Stack
            </h2>
            <div className="flex flex-wrap gap-2 mb-12">
              {project.stack.map((tech) => (
                <span
                  key={tech}
                  className="px-4 py-2 bg-zinc-900 border border-white/5 rounded-full text-xs text-zinc-300"
                >
                  {tech}
                </span>
              ))}
            ...
            <a
              href={project.url}
              target="_blank"
              className="flex items-center justify-between p-6 bg-white text-black rounded-2xl font-bold hover:bg-blue-500 hover:text-white transition-all"
            >
